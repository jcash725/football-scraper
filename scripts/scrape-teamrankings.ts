...
type Output = {
  ...
  note?: string | null;
  columns: string[];
  rows: Row[]; // sorted most -> least by the chosen numeric column
};
...
async function scrapeOne(target: { id: string; url: string }): Promise<Output> {
  ...
  if (!res.ok) throw new Error(`Fetch failed ${res.status} for ${target.url}`);
  ...
  const rows: Row[] = rowCells.map(cells => {
    const obj: Row = {};
  ...
  rows.sort((a, b) => {
    const av = typeof a[sortHeader] === "number" ? (a[sortHeader] as number) : -Infinity;
    const bv = typeof b[sortHeader] === "number" ? (b[sortHeader] as number) : -Infinity;
    return bv - av;
  });
