...
const STATMUSE_TEAM_MAP: Record<string, string> = {
  "Philadelphia": "eagles",
  "Dallas": "cowboys",
  "Kansas City": "chiefs", 
...
function getStatMuseTeamName(shortName: string): string {
  return STATMUSE_TEAM_MAP[shortName] || shortName.toLowerCase();
}

async function getPlayerTDsVsOpponent(playerName: string, team: string, opponent: string, tdType: 'rushing' | 'receiving'): Promise<string> {
  try {
...
function getOpponent(team: string, matchups: Matchup[]): string | null {
...
function getOpponentStat(opponent: string, stats: TeamStat[]): number {
...
async function generateTop20List(): Promise<TDRecommendation[]> {
  const data = loadData();
...
function formatAsTable(recommendations: TDRecommendation[]): string {
...
async function main() {
  try {
    ...
    fs.writeFileSync(
      path.join(outputDir, 'td-bet-recommendations.json'),
      JSON.stringify({
    ...
    fs.writeFileSync(
      path.join(outputDir, 'td-bet-recommendations.md'),
      tableOutput
    );
